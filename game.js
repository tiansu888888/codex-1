...
const chars = {
  '1': { name: 'Gintoki', speed: 0.9, jump: 14, damage: 1, color: '#d9f3ff' },
  '2': { name: 'Kagura', speed: 1.15, jump: 13, damage: 1.4, color: '#ffc0d3' },
  '3': { name: 'Shinpachi', speed: 0.82, jump: 15, damage: 0.9, color: '#cce1ff' },
};

const world = { width: 3200, gravity: 0.68, friction: 0.84 };
const player = { x: 100, y: 420, w: 36, h: 56, vx: 0, vy: 0, face: 1, hp: 5, sp: 100, coins: 0, atk: 0, dash: 0, onGround: false, char: '1' };
const platforms = [
...
function draw() {
  ...
  ctx.fillStyle = player.atk > 0 ? '#ffd59a' : chars[player.char].color; ctx.fillRect(player.x, player.y, player.w, player.h);
  if (player.atk > 0) { ctx.fillStyle = 'rgba(255,220,120,.55)'; const ax = player.face > 0 ? player.x + player.w : player.x - 30; ctx.fillRect(ax, player.y + 8, 30, 24); }
  ctx.restore();
